<script setup lang="ts">
  ...
  const { sneakers: sneakersStore, favorites, setFavorites, setSneakers, sortBy, changeSort } = useSneakersStore()

  const items = ref<sneaker[]>(props.favorited ? sneakersStore.value.filter((sneaker) => sneaker.isFavorite) : sneakersStore.value)
  ...
  watch(searchQuery, (searchQuery) => {
    items.value = props.favorited ? sneakersStore.value.filter((sneaker) => sneaker.isFavorite) : sneakersStore.value
    if (searchQuery !== '') {
  ...
  const favoriteHandler = async (id: number) => {
    try {
      ...
      const newFavorites = await updateFavorites(includes ? favorites.value.filter(num => num !== id) : [...favorites.value, id])
  ...
  <section class="p-10">
    <div class="flex justify-between">
      ...
      <div class="flex gap-4">
        <select @change="sortHandler" :value="sortBy" class="py-2 px-3 border rounded-md outline-none h-[45px]">
        ...
        <div class="relative">
          ...
          <input @input="searchHandler" class="border border-gray-200 rounded-md py-2 pl-[46px] pr-1 h-[45px] outline-none focus:border-gray-400" placeholder="Поиск...">
        </div>
      </div>
    </div>
    <div v-auto-animate class="grid grid-cols-4 gap-5">
      <CardItem v-for="item in items" v-bind:key="item.id" :id="item.id" :image-url="item.imageUrl" :title="item.title" :price="item.price" :is-favorite="item.isFavorite" :is-added="cart.has(item.id)" @onClickAdd="() => addHandler(item)" @onClickFavorite="favoriteHandler" />
    </div>
  </section>
</template>
