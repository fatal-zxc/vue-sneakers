...
<template>
  <div v-auto-animate class="flex scrollbar overflow-y-auto max-h-[600px] flex-col flex-1 gap-4">
    <CartItem v-for="sneaker in cart.values()" v-bind:key="sneaker.id" @delete-item="() => deleteItem(sneaker)" :image-url="sneaker.imageUrl" :title="sneaker.title" :price="sneaker.price"/>
  </div>
</template>